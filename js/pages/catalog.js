...
async function _fetchPage1AndUpdate(currentUser, query, page) {
    try {
        ...
        // isAuthError → sesi/token sudah tidak valid. Modal login ulang
        // sudah otomatis terbuka lewat listener "ajp:auth-expired" di
        // app.js, jadi di sini cukup tampilkan data cache (kalau ada)
        // dengan pesan yang akurat — bukan "periksa koneksi internet"
        // yang menyesatkan padahal internetnya baik-baik saja.
...
async function _backgroundFetchAll(currentUser, page1Result) {
    ...
    if (!pagination || pagination.totalPages <= 1) {
        // Ambil snapshot LAMA dulu SEBELUM ditimpa — kalau tidak, diff
        // di bawah akan membandingkan data baru dengan data baru (yang
        // barusan disimpan), tidak akan pernah ketemu "produk baru".
        ...
        await _runCatalogDiff(currentUser, page1Result.products, previous?.products);
        return;
    }
    ...
    try {
        const params = { allForCache: "true" };
        if (currentUser?.role !== "public") {
            params.priceArea = _getActivePriceArea(currentUser);
        }

        // Satu request ambil SEMUA produk sekaligus (backend sudah
        // mendukung ini lewat allForCache=true). Sebelumnya di sini ada
        // loop yang fetch per-halaman satu-satu (bisa 80-125+ request
        // berurutan untuk katalog 2.000-3.000 baris, ditambah jeda 300ms
        // tiap halaman) — selain lambat (bisa >1 menit), kalau tab
        // ditutup sebelum selesai maka produk di halaman yang belum
        // sempat ke-fetch TIDAK PERNAH dicek harganya sama sekali, jadi
        // perubahan harga di sana tidak akan pernah muncul sebagai
        // notifikasi. Satu request besar jauh lebih cepat & tidak bisa
        // "terpotong di tengah".
        const result = await API.getProducts(params);
        ...
        if (allProducts.length > 0) {
            ...
            // Sama seperti di atas: ambil snapshot LAMA dulu sebelum
            // saveFull() menimpanya, supaya _runCatalogDiff benar-benar
            // membandingkan lama vs baru.
            const previous = await CatalogStorage.loadFull(currentUser);

            await CatalogStorage.saveFull(
                currentUser,
                allProducts,
                result.priceArea || page1Result.priceArea || "regular",
                result.filters || page1Result.filters
            );

            _runPriceCheck(currentUser, allProducts);

            await _runCatalogDiff(currentUser, allProducts, previous?.products);

        }

    } catch (e) {
        console.warn("Background full-fetch gagal:", e);
    } finally {
...
async function _runCatalogDiff(currentUser, newProducts, previousProducts) {
    const isRealUser = currentUser &&
        currentUser.role !== "public" &&
        currentUser.role !== "guest";
    ...
    try {
        // Kalau pemanggil sudah kasih snapshot lama secara eksplisit,
        // pakai itu. Kalau tidak (dipanggil dari tempat lain), baru
        // fallback baca dari cache — TAPI ini hanya valid kalau dipanggil
        // SEBELUM cache ditimpa dengan data baru.
        const previous = previousProducts !== undefined
            ? previousProducts
            : (await CatalogStorage.loadFull(currentUser))?.products;

        const changes  = diffCatalog(previous, newProducts)
...
async function runBackgroundPriceSync(currentUser) {
    ...
    try {
        // PENTING: ambil data FRESH dari server lewat allForCache=true —
        // BUKAN dari CatalogStorage.loadFull() (cache lokal) seperti
        // sebelumnya. Cache lokal itu SENDIRI diisi dari hasil
        // PriceHistory.checkAndUpdate() sebelumnya (lihat
        // _backgroundFetchAll), jadi membandingkannya ke situ lagi sama
        // saja membandingkan data ke dirinya sendiri — tidak akan PERNAH
        // ketemu perbedaan apa pun. Ini penyebab utama notifikasi harga
        // tidak muncul walau harga di card sudah ter-update.
        const params = { allForCache: "true" };
        if (currentUser.role !== "public") {
            params.priceArea = _getActivePriceArea(currentUser);
        }
        ...
        await PriceHistory.checkAndUpdate(result.products);

    } catch (e) {
        console.warn("Background price sync gagal:", e);
    }
}
